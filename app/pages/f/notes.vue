...
<template>
  <FeatureGuard feature="notes">
    <div class="max-w-xl">
      <h1 class="mb-5 text-2xl font-semibold tracking-tight text-highlighted">速记</h1>
      ...
      <div class="mb-4 flex gap-2">
        <UTextarea
          ...
          class="flex-1"
        />
        <UButton :loading="saving" color="neutral" label="添加" @click="add" />
      </div>

      <div
        v-if="!notes?.length"
        class="rounded-2xl border border-dashed border-default py-12 text-center text-muted"
      >
        还没有任何速记
      </div>

      <div v-else class="divide-y divide-zinc-200/60 overflow-hidden rounded-2xl bg-default shadow-sm ring-1 ring-zinc-200/70">
        <div v-for="n in notes" :key="n.id" class="flex items-start justify-between gap-3 p-4 transition hover:bg-elevated/40">
          <div>
